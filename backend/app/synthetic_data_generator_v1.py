...
def generate():
    ...
    for idx, scenario in enumerate(scenarios, start=1):
        ...
        for payment_index, current_payment_id in enumerate(payment_ids):
            ...
            if scenario == "GATEWAY_FEE_GST":
                ...
                gateway_fee = money(gross_amount * 0.02)
                gst_on_fee = money(gateway_fee * 0.18)

            # Full refund
            if scenario == "FULL_REFUND" and payment_index == 0:
            ...
            # Partial refund
            if scenario == "PARTIAL_REFUND" and payment_index == 0:

                refund_adjustment = money(
                    gross_amount
                    * random.choice([0.20, 0.25, 0.30, 0.40])
                )
            ...
            # Chargeback
            if scenario == "CHARGEBACK" and payment_index == 0:
            ...
            if scenario == "UNRESOLVED":
                ...
                actual_bank_amount = max(
                    0,
                    expected_net
                    - random.choice([3000, 5000, 8000, 12000])
                )
            ...
            bank_reference = (
                f"BANK-{settlement_id}"
            )

            # Fuzzy bank reference
            if scenario == "FUZZY_REFERENCE":
    ...
    datasets = {
        ...
        "ground_truth.csv": pd.DataFrame(ground_truth),
    }
    ...
    print()
    print("FlowGuard synthetic dataset generated successfully.")
    print()
    ...
    ground_truth_df = datasets["ground_truth.csv"]
